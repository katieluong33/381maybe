...
df$G3_bin <- cut(df$G3, breaks = 4, labels = FALSE)

#df$strata <- interaction(df$G3_bin, df$activities, df$class)

### Split the data - training and remaining
train_index <- createDataPartition(df$class, p = 0.70, list = FALSE)
train_data <- df[train_index, ]
remaining <- df[-train_index, ]

### Split the data - validation and test
valid_index <- createDataPartition(remaining$class, p = 0.5, list = FALSE)
...
ggplot() +
  ...
  geom_point(aes(x = train_data$G2, y = train_data$G3, color = "Train Actual"), alpha = 0.5) +  # Training data
  geom_point(aes(x = valid_data$G2, y = valid_data$G3, color = "Validation Actual"), alpha = 0.5) +   # Validation data
  
  # Model 1 (Bivariate model): solid for training, dashed for validation
  geom_line(aes(x = train_data$G2, y = bivariate_pred_train, color = "Bivariate Model"), linetype = "solid") +
  geom_line(aes(x = valid_data$G2, y = bivariate_pred_valid, color = "Bivariate Model"), linetype = "dashed") +
  
  # Model 2 (Bivariate with nonlinearity): solid for training, dashed for validation
  geom_line(aes(x = train_data$G2, y = bivariate_2_pred_train, color = "Nonlinearity Model"), linetype = "solid") +
  geom_line(aes(x = valid_data$G2, y = bivariate_2_pred_valid, color = "Nonlinearity Model"), linetype = "dashed") +
  
  # Model 3 (Bivariate with nonlinearity and lasso): solid for training, dashed for validation
  geom_line(aes(x = train_data$G2, y = bivariate_3_pred_train, color = "Lasso Model"), linetype = "solid") +
  geom_line(aes(x = valid_data$G2, y = bivariate_3_pred_valid, color = "Lasso Model"), linetype = "dashed") +
  
  # Model 4 (Spline model): solid for training, dashed for validation
  geom_line(aes(x = train_data$G2, y = spline_pred_train, color = "Spline Model"), linetype = "solid") +
  geom_line(aes(x = valid_data$G2, y = spline_pred_valid, color = "Spline Model"), linetype = "dashed") +
